...
void initializeGrids(int argn, char** argc, dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid,
   ...
   mpiGrid.set_initial_length(grid_length)
       ...
       .set_periodic(sysBoundaries.isPeriodic(0), sysBoundaries.isPeriodic(1), sysBoundaries.isPeriodic(2))
       .initialize(comm)
   ...
   computeCoupling(mpiGrid, cells, technical, fsgrid);
   ...
   if (P::isRestart) {
      ...
      if (readGrid(mpiGrid, perb.view(), e.view(), technical, fsgrid, P::restartFileName) == false) {
      ...
      if (P::forceRefinement) {
         ...
         for (int i = 0; i < P::amrMaxSpatialRefLevel; ++i) {
            ...
            if (!adaptRefinement(mpiGrid, technical, fsgrid, sysBoundaries, project, i)) {
               cerr << "(MAIN) ERROR: Forcing refinement takes too much memory" << endl;
               exit(1);
            }
            balanceLoad(mpiGrid, sysBoundaries, technical, fsgrid);
         }
         ...
         balanceLoad(mpiGrid, sysBoundaries, technical, fsgrid);
         adaptRefinement(mpiGrid, technical, fsgrid, sysBoundaries, project);
         balanceLoad(mpiGrid, sysBoundaries, technical, fsgrid);
      }
   }
   ...
   if (P::isRestart) {
      ...
      sysBoundaries.applyInitialState(mpiGrid, technical, fsgrid, perb.view(), bgb.view(), project);
   }
...
#pragma omp parallel for schedule(static)
      ...
      */
      ...
      // If, instead of starting a regular simulation, we are only writing out the background field, it is enough to set
      // a dummy load balance value of 1 here.
      for (size_t i = 0; i < cells.size(); ++i) {
   ...
   balanceLoad(mpiGrid, sysBoundaries, technical, fsgrid, false);
   ...
   project.setProjectBField(perb.view(), bgb.view(), technical, fsgrid);
   setBTimer.stop();
   phiprof::Timer fsGridGhostTimer{"fsgrid-ghost-updates"};
   fsgrid.updateGhostCells(perb);
   fsgrid.updateGhostCells(bgb);
   ...
   feedMomentsIntoFsGrid(mpiGrid, cells, momentsdt2, technical.view(), fsgrid, P::isRestart);
...
void prepareAMRLists(dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid) {
   ...
   if (P::vlasovSolverGhostTranslate) {
      ...
      const vector<CellID> remote_cells =
          mpiGrid.get_remote_cells_on_process_boundary(VLASOV_SOLVER_GHOST_REQNEIGH_NEIGHBORHOOD_ID);
      // const vector<CellID> remote_cells =
...
void report_grid_memory_consumption(dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid) {
   ...
   logFile << "(MEM)   Average capacity: " << sum_mem[5] / n_procs << " local cells " << sum_mem[3] / n_procs
           << " remote cells " << sum_mem[4] / n_procs << endl;
...
void deallocateRemoteCellBlocks(dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid) {
   const std::vector<uint64_t> remote_cells = mpiGrid.get_remote_cells_on_process_boundary(FULL_NEIGHBORHOOD_ID);
   for (unsigned int i = 0; i < remote_cells.size(); i++) {
...
DIST_FUNC  (Includes all cells which should know about each others blocks and have space for them. VLASOV +
SYSBOUNDARIES.
-----------
    x
   xxx
...
#pragma omp parallel for
      for (size_t c = 0; c < cells.size(); ++c) {
         ...
         // #warning TODO should VAMR grandparents be checked only for face neighbors instead of
...
#pragma omp for
            for (size_t b = 0; b < newBlocks[c].size(); ++b) {
               if (getObjectWrapper().project->setVelocityBlock(cell, newBlocks[c][b].second, popID) <=
                   cell->getVelocityBlockMinValue(popID)) {
...
bool adaptRefinement(dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid,
   ...
   logFile << "(AMR) Refining " << refines << " cells after induces, " << 100.0 * ratio_refines << "% of grid"
           << std::endl;
   logFile << "(AMR) Coarsening " << coarsens << " cells after induces, " << 100.0 * ratio_coarsens << "% of grid"
           << std::endl;
   ...
   computeCoupling(mpiGrid, cellsVec, technical, fsgrid);
