 ...
 * @return Returns true if the operation was successful
 @ @see exec_readGrid
*/
bool readNBlocks(vlsv::ParallelReader& file, const std::string& meshName, std::vector<size_t>& nBlocks, int masterRank,
                 MPI_Comm comm) {
   bool success = true;
 ...
 * @param blocksPerCell Number of velocity blocks for this particle species in each spatial cell belonging to this
 * process.
...
bool _readBlockData(vlsv::ParallelReader& file, const std::string& spatMeshName, const std::vector<uint64_t>& fileCells,
                    const uint64_t localCellStartOffset, const uint64_t localCells, const vmesh::LocalID* blocksPerCell,
                    const uint64_t localBlockStartOffset, const uint64_t localBlocks,
                    dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid,
                    std::function<vmesh::GlobalID(vmesh::GlobalID)> blockIDremapper, const uint popID) {
   uint64_t arraySize;
...
bool readBlockData(vlsv::ParallelReader& file, const string& meshName, const vector<CellID>& fileCells,
   ...
   for (uint popID = 0; popID < getObjectWrapper().particleSpecies.size(); ++popID) {
      ...
      if (fileMeshBBox[0] != ourMeshParams.gridLength[0] || fileMeshBBox[1] != ourMeshParams.gridLength[1] ||
         ...
         for (const auto& c : fileVelCoordsX) {
            ...
            if (fabs(nearbyint(cellindex) - cellindex) > 1. / 10000.) {
               logFile << "(RESTART) ERROR: Can't resize velocity space as cell coordinates don't match." << endl
                       ...
                       << getObjectWrapper().velocityMeshes[meshID].meshMinLimits[0] << endl
         ...
         for (const auto& c : fileVelCoordsY) {
            ...
            if (fabs(nearbyint(cellindex) - cellindex) > 1. / 10000.) {
               logFile << "(RESTART) ERROR: Can't resize velocity space as cell coordinates don't match." << endl
                       ...
                       << getObjectWrapper().velocityMeshes[meshID].meshMinLimits[1] << endl
                       << "           coordinate  = cellindex *   dV  +  meshMinLimits)" << endl
                       << write;
               abort();
            }
         ...
         for (const auto& c : fileVelCoordsY) {
            ...
            if (fabs(nearbyint(cellindex) - cellindex) > 1. / 10000.) {
               logFile << "(RESTART) ERROR: Can't resize velocity space as cell coordinates don't match." << endl
                       ...
                       << getObjectWrapper().velocityMeshes[meshID].meshMinLimits[2] << endl
                       << "           coordinate  = cellindex *   dV  +  meshMinLimits)" << endl
                       << write;
               abort();
            }
      ...
      if (dataType == vlsv::datatype::type::FLOAT) {
         ...
         case sizeof(double):
            if (_readBlockData<double>(file, meshName, fileCells, localCellStartOffset, localCells, blocksPerCell,
                                       myOffset, blockSum, mpiGrid, blockIDremapper, popID) == false)
               success = false;
            break;
         case sizeof(float):
...
bool exec_readGrid(dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid,
                   ...
                   fsgrid::FsGrid< FS_STENCIL_WIDTH>& fsgrid, const std::string& name) {
   ...
   if (success) {
      success = readFsGridVariable(file, "fg_PERB", fsgridInputRanks, fsgrid, perb);
   }
   if (success) {
      success = readFsGridVariable(file, "fg_E", fsgridInputRanks, fsgrid, e);
   }
...
bool readGrid(dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid,
   ...
   return exec_readGrid(mpiGrid, perb, e, fsgrid, name);
}
