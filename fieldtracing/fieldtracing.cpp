 ...
 * \brief Implementation of the field tracing algorithms used in Magnetosphere runs and in magnetosphere-ionosphere
 * coupling.
 */
...
void reduceData(fsgrid::FsGrid<fsgrids::technical, FS_STENCIL_WIDTH>& technicalGrid,
   ...
   if (fieldTracingParameters.doTraceOpenClosed) {
      traceOpenClosedConnection(technicalGrid, perb, dperb, nodes);
   }
   if (fieldTracingParameters.doTraceFullBox) {
      traceFullBoxConnectionAndFluxRopes(technicalGrid, perb, dperb, mpiGrid);
   }
}
...
void calculateIonosphereFsgridCoupling(fsgrid::FsGrid<fsgrids::technical, FS_STENCIL_WIDTH>& technicalGrid,
   ...
   std::vector<Real> nodeTracingStepSize(
   ...
   std::vector<Real> nodeDistance(
       nodes.size(), std::numeric_limits<Real>::max()); // For reduction of node coordinate in case of multiple hits
   std::vector<int> nodeNeedsContinuedTracing(nodes.size(),
                                              1); // Flag, whether tracing needs to continue on another task
   std::vector<std::array<Real, 3>> nodeTracingCoordinates(
       nodes.size()); // In-flight node upmapping coordinates (for global reduction)
   ...
   TracingFieldFunction<Real> tracingFullField =
       [&perb, &dperb, &technicalGrid](std::array<Real, 3>& r, const bool alongB, std::array<Real, 3>& b) -> bool {
      return traceFullFieldFunction(perb, dperb, technicalGrid, r, alongB, b);
   };
         ...
         for (uint n = 0; n < nodes.size(); n++) {
            ...
            while (true) {
               ...
               if (x[0] * x[0] + x[1] * x[1] + x[2] * x[2] >
                   SBC::Ionosphere::downmapRadius * SBC::Ionosphere::downmapRadius) {
                  ...
                  const std::array<Real, 3> perB = interpolatePerturbedB(
                      perb, dperb, technicalGrid, fieldTracingParameters.reconstructionCoefficientsCache, fsgridCell[0],
                  ...
                  no.parameters[ionosphereParameters::UPMAPPED_BY] =
                      SBC::ionosphereGrid.dipoleField(x[0], x[1], x[2], Y, 0, Y) + SBC::ionosphereGrid.BGB[1] + perB[1];
                  no.parameters[ionosphereParameters::UPMAPPED_BZ] =
                      SBC::ionosphereGrid.dipoleField(x[0], x[1], x[2], Z, 0, Z) + SBC::ionosphereGrid.BGB[2] + perB[2];

                  no.haveCouplingData = 1;
                  nodeDistance[n] = sqrt((no.xMapped[0] - no.x[0]) * (no.xMapped[0] - no.x[0]) +
                                         (no.xMapped[1] - no.x[1]) * (no.xMapped[1] - no.x[1]) +
                                         (no.xMapped[2] - no.x[2]) * (no.xMapped[2] - no.x[2]));
                  nodeNeedsContinuedTracing[n] = 0;
                  nodeTracingCoordinates[n] = {0, 0, 0};
                  break;
               }
...
calculateIonosphereVlasovGridCoupling(std::array<Real, 3> x, std::vector<SBC::SphericalTriGrid::Node>& nodes,
   ...
   while (x[0] * x[0] + x[1] * x[1] + x[2] * x[2] > SBC::Ionosphere::innerRadius * SBC::Ionosphere::innerRadius) {
      ...
      // Make one step along the fieldline
      stepFieldLine(x, v, stepSize, fieldTracingParameters.min_tracer_dx_ionospere_coupling,
                    fieldTracingParameters.max_tracer_dx_ionospere_coupling, fieldTracingParameters.tracingMethod,
                    dipoleFieldOnly, false);
      ...
      if (x[0] * x[0] + x[1] * x[1] + x[2] * x[2] > 1.5 * 1.5 * couplingRadius * couplingRadius) {
         cerr << "(fieldtracing) Warning: coupling of Vlasov grid cell failed due to weird magnetic field topology."
              << endl;
   ...
   for (uint toto = 0; toto < 15; toto++) {
      ...
      if (elementHistory.find(elementIndex) == elementHistory.end()) {
         ...
         // It happens when the projection rx is left seen from the right triangle and right seen from the left
         // triangle.
      ...
      if (override || (kappa1 > 0 && kappa2 > 0 && kappa3 > 0)) {
         ...
         cerr << "This fell through, strange."
              << " kappas " << kappa1 << " " << kappa2 << " " << kappa3 << " handedness " << handedness << " r1 "
              << r1[0] << " " << r1[1] << " " << r1[2] << " r2 " << r2[0] << " " << r2[1] << " " << r2[2] << " r3 "
              << r3[0] << " " << r3[1] << " " << r3[2] << " rx " << rx[0] << " " << rx[1] << " " << rx[2] << endl;
      }
      if (elementIndex == -1) {
         cerr << __FILE__ << ":" << __LINE__ << ": invalid elementIndex returned for coordinate " << x[0] << " " << x[1]
              << " " << x[2] << " projected to rx " << rx[0] << " " << rx[1] << " " << rx[2]
   ...
   cerr << "(fieldtracing) Failed to find an ionosphere element to couple to for coordinate " << x[0] << " " << x[1]
        << " " << x[2] << endl;
   return coupling;
}
...
void traceOpenClosedConnection(fsgrid::FsGrid<fsgrids::technical, FS_STENCIL_WIDTH>& technicalGrid,
   ...
   std::vector<int> nodeNeedsContinuedTracing(nodes.size(),
                                              1); /*!< Flag, whether tracing needs to continue on another task */
   std::vector<std::array<TReal, 3>> nodeTracingCoordinates(
       nodes.size()); /*!< In-flight node upmapping coordinates (for global reduction) */
   ...
   TracingFieldFunction<TReal> tracingFullField =
       [&perb, &dperb, &technicalGrid](std::array<TReal, 3>& r, const bool alongB, std::array<TReal, 3>& b) -> bool {
      return traceFullFieldFunction(perb, dperb, technicalGrid, r, alongB, b);
   };
         ...
         for (uint n = 0; n < nodes.size(); n++) {
            ...
            while (true) {
               ...
               std::array<fsgrid::FsIndex_t, 3> fsgridCell =
                   getLocalFsGridCellIndexForCoord(technicalGrid, {(TReal)x[0], (TReal)x[1], (TReal)x[2]});
               // If it is not in our domain, somebody else takes care of it.
               if (fsgridCell[0] == -1) {
                  nodeNeedsContinuedTracing[n] = 0;
                  nodeTracingCoordinates[n] = {0, 0, 0};
...
#pragma omp critical
               ...
               if (x[0] > P::xmax - 4 * P::dx_ini || x[0] < P::xmin + 4 * P::dx_ini || x[1] > P::ymax - 4 * P::dy_ini ||
                   x[1] < P::ymin + 4 * P::dy_ini || x[2] > P::zmax - 4 * P::dz_ini || x[2] < P::zmin + 4 * P::dz_ini) {
                  nodeNeedsContinuedTracing[n] = 0;
                  nodeTracingCoordinates[n] = {0, 0, 0};
   ...
   if (redWarning && rank == MASTER_RANK) {
      logFile << "(fieldtracing) Warning: reached the maximum number of tracing steps " << maxTracingSteps
              << " allowed for open-closed ionosphere tracing." << endl;
   }
 ...
 */
void stepCellAcrossTaskDomain(cint n, fsgrid::FsGrid<fsgrids::technical, FS_STENCIL_WIDTH>& technicalGrid,
                              TracingFieldFunction<TReal>& tracingFullField,
   ...
   while (true) {
      // Check if the current coordinates (pre-step) are in our own domain.
      std::array<fsgrid::FsIndex_t, 3> fsgridCell =
          getLocalFsGridCellIndexForCoord(technicalGrid, {(Real)x[0], (Real)x[1], (Real)x[2]});
      // If it is not in our domain, somebody else takes care of it.
      if (fsgridCell[0] == -1) {
      ...
      if (x.at(0) * x.at(0) + x.at(1) * x.at(1) + x.at(2) * x.at(2) <
          fieldTracingParameters.innerBoundaryRadius * fieldTracingParameters.innerBoundaryRadius) {
         ...
         Real r_in = sqrt(cellTracingCoordinates[n][0] * cellTracingCoordinates[n][0] +
...
#pragma omp critical
      ...
      if (cellConnection[n] < TracingLineEndType::N_TYPES) {
         const TReal extension =
             sqrt((x[0] - (cellInitialCoordinates[n])[0]) * (x[0] - (cellInitialCoordinates[n])[0]) +
         ...
         } else if (cellRunningDistance[n] >
                    fieldTracingParameters.fluxrope_max_curvature_radii_to_trace * cellCurvatureRadius[n]) {
...
/*!< \brief Trace magnetic field lines forward and backward from each DCCRG cell to record the connectivity and detect
 * flux ropes.
 *
 ...
 *
 * As a freebie since we computed the curvature anyway for flux rope tracing we store that into
 * CellParams::CURVATUREX/Y/Z.
 *
...
void traceFullBoxConnectionAndFluxRopes(fsgrid::FsGrid<fsgrids::technical, FS_STENCIL_WIDTH>& technicalGrid,
   ...
   // Pick an initial stepsize
   const TReal stepSize = min(1000e3, technicalGrid.getGridSpacing()[0] / 2.);
   std::vector<TReal> cellFWTracingStepSize(
       globalDccrgSize, stepSize); // In-flight storage of step size, needed when crossing into next MPI domain
   std::vector<TReal> cellBWTracingStepSize(
       globalDccrgSize, stepSize); // In-flight storage of step size, needed when crossing into next MPI domain

   std::array<fsgrid::FsSize_t, 3> gridSize = technicalGrid.getGlobalSize();
   // If fullbox_and_fluxrope_max_distance is unset, use this heuristic considering how far an IMF+dipole combo can
   // sensibly stretch in the box before we're safe to assume it's rolled up more or less pathologically.
   ...
   std::vector<signed char> cellFWConnection(globalDccrgSize,
                                             TracingLineEndType::UNPROCESSED); /*!< For reduction of node coupling */
   std::vector<signed char> cellBWConnection(globalDccrgSize,
                                             TracingLineEndType::UNPROCESSED); /*!< For reduction of node coupling */

   std::vector<std::array<TReal, 3>> cellFWTracingCoordinates(
       globalDccrgSize); /*!< In-flight node upmapping coordinates (for global reduction) */
   std::vector<std::array<TReal, 3>> cellBWTracingCoordinates(
       globalDccrgSize); /*!< In-flight node upmapping coordinates (for global reduction) */
   ...
   for (int n = 0; n < globalDccrgSize; n++) {
      ...
      if (mpiGrid.is_local(id)) {
         if ((mpiGrid[id]->sysBoundaryFlag != sysboundarytype::NOT_SYSBOUNDARY) ||
            ...
            cellCurvatureRadius[n] =
                1 /
                sqrt(mpiGrid[id]->parameters[CellParams::CURVATUREX] * mpiGrid[id]->parameters[CellParams::CURVATUREX] +
                     mpiGrid[id]->parameters[CellParams::CURVATUREY] * mpiGrid[id]->parameters[CellParams::CURVATUREY] +
                     mpiGrid[id]->parameters[CellParams::CURVATUREZ] * mpiGrid[id]->parameters[CellParams::CURVATUREZ]);
            if (fieldTracingParameters.fluxrope_max_curvature_radii_to_trace * cellCurvatureRadius[n] >
                maxTracingDistance) {
               cellCurvatureRadius[n] =
                   0; // This will stop fluxrope tracing for these field lines in the first iteration below.
            }
         }
      }
   }
   ...
   // The FW, BW and this copy of coordinates were created using mpiGrid.get_center() which is for all cells, not just
   // local ones, so no need to reduce them now.
...
#pragma omp single
         { itCount++; }
// Trace node coordinates forward and backwards until a non-sysboundary cell is encountered or the local fsgrid domain
// has been left.
#pragma omp for schedule(dynamic)
         for (int n = 0; n < globalDccrgSize; n++) {
...
#pragma omp master
         {
            ...
            for (int n = 0; n < globalDccrgSize; n++) {
               if (storedCellFWConnection[n] % TracingLineEndType::N_TYPES ==
                   TracingLineEndType::UNPROCESSED) { // This still has the allreduce MPI_SUM results from the previous
                                                      // round, so that all globally continuing cells are accounted for.
               ...
               if (storedCellBWConnection[n] % TracingLineEndType::N_TYPES ==
                   TracingLineEndType::UNPROCESSED) { // This still has the allreduce MPI_SUM results from the previous
                                                      // round, so that all globally continuing cells are accounted for.
...
#pragma omp for schedule(dynamic) reduction(+ : cellsToDoFullBox) reduction(+ : cellsToDoFluxRopes)
         for (int n = 0; n < globalDccrgSize; n++) {
            if (cellFWConnection[n] % TracingLineEndType::N_TYPES == TracingLineEndType::UNPROCESSED ||
                cellBWConnection[n] % TracingLineEndType::N_TYPES == TracingLineEndType::UNPROCESSED) {
               cellsToDoFullBox++;
               if (cellFWConnection[n] == TracingLineEndType::UNPROCESSED ||
                   cellBWConnection[n] == TracingLineEndType::UNPROCESSED) {
                  cellsToDoFluxRopes++;
               }
            }
         }
#pragma omp barrier
      } while (!(cellsToDoFullBox <= fieldTracingParameters.fullbox_max_incomplete_cells * globalDccrgSize &&
   ...
   logFile << "(fieldtracing) combined flux rope + full box tracing traced in " << itCount
           << " iterations of the tracing loop with flux rope " << cellsToDoFluxRopes << ", full box "
           << cellsToDoFullBox << " remaining incomplete field lines (total spatial cells " << globalDccrgSize << ")."
           << endl;
   ...
   MPI_Comm_rank(MPI_COMM_WORLD, &rank);
   if (redWarning && rank == MASTER_RANK) {
      logFile << "(fieldtracing) Warning: reached the maximum tracing distance " << maxTracingDistance
   ...
   for (int n = 0; n < globalDccrgSize; n++) {
      ...
      if (mpiGrid.is_local(id)) {
         ...
         // Earlier, if we marked nothing (e.g. hit a wall or ionosphere before making a call) cellXWConnection[n] is
         // less than N_TYPES. If we went beyond the thresholds we did += N_TYPES, which is also not a positive hit. If
         // we identified a flux rope we did a double += by N_TYPES and we pick them out with this.
         ...
         if (cellFWConnection[n] == TracingLineEndType::DANGLING &&
             cellBWConnection[n] == TracingLineEndType::DANGLING) {
