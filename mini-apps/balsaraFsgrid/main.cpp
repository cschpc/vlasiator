...
int main(int argc, char** argv) {
   ...
   fsgrid::FsGrid<fsgrids::technical, FS_STENCIL_WIDTH> fsgrid(
   ...
   fsgrid::FsGrid<std::array<Real, fsgrids::dperb::N_DPERB>, FS_STENCIL_WIDTH> dPerBGrid(
       fsGridDimensions, parentComm, numFsProcs, periodicity, gridSpacing, physicalGlobalStart, decomposition);
   std::span<fsgrids::technical> technical = fsgrid.getData();
   ...
   for (int i = 0; i < 5; i++) {
      for (int j = 0; j < 5; j++) {
         for (int k = 0; k < 5; k++) {
            const auto stencil = fsgrid.makeStencil(i, j, k);
            calculateDerivatives(stencil, perb, dperb, fsgrid);
         }
      }
   }
   ...
   for (int i = 0; i < 1000; i++) {
      ...
      std::array<Real, 3> B = interpolatePerturbedB(perBGrid, dPerBGrid, fsgrid, cache, fsgridCell[0],
